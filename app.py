...
with tab1:
    ...
    meter_sel = st.selectbox("Choose meter ID for explanation", options=sorted(df['Meter_ID'].unique()))
    # compute features for this meter
    meter_df = df[df['Meter_ID']==meter_sel].copy()
    # naive feature extraction (reuse earlier create_features logic if available)
    feat = {}
    feat['night_day_ratio'] = (meter_df[meter_df['Timestamp'].dt.hour.between(0,5)]['Consumption_kWh'].mean() / 
    ...
    # prediction: use existing model if loaded, else use simple heuristic
    try:
        ...
        if 'model' in globals():
            ...
            prob = 0.5 if feat['zero_consumption_hours']>10 or feat['night_day_ratio']<0.2 else 0.05
...
with tab2:
    ...
    meter_f = st.selectbox("Select meter for forecast", options=sorted(df['Meter_ID'].unique()), index=0)
    meter_df = df[df['Meter_ID']==meter_f][['Timestamp','Consumption_kWh']].copy()
...
with tab3:
    ...
    if st.button("Run clustering and train fast ensemble"):
        # create feature matrix per meter using basic stats
        meters = []
        feats = []
        for mid, group in df.groupby('Meter_ID'):
            meters.append(mid)
        ...
        # train ensemble on existing labeled data if Theft_Flag exists
        if 'Theft_Flag' in df.columns:
            # prepare X,y by aggregating (simple)
            X = feat_df.fillna(0)
            y = []
            for mid in feat_df.index:
                # label by mode of theft flag for that meter
                lab = df[df['Meter_ID']==mid]['Theft_Flag'].mode()
                y.append(int(lab.iloc[0]) if not lab.empty else 0)
            models_local = ai_modules.train_ensemble(X, np.array(y), use_xgb=False)
            st.session_state['ensemble_models'] = models_local
            st.success("Trained fast ensemble (RF).")
        else:
            st.info("No Theft_Flag present in data to supervise training.")

    if 'ensemble_models' in st.session_state:
        st.write("Ensemble ready. You can run predictions on meters.")
        models_local = st.session_state['ensemble_models']
        if st.button("Predict sample meters"):
            sample_feats = feat_df.fillna(0).iloc[:5]
            probs, votes = ai_modules.predict_ensemble(models_local, sample_feats)
...
with tab4:
    ...
    if st.button("Start simulation (50 steps)"):
        ...
        alerts = []
        for item in gen:
            # simple rule: if consumption < 20% of mean, raise alert
            mid = item['Meter_ID']; val = item['Consumption_kWh']
